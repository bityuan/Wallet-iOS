...
public enum AssetAttributeSyntax: String {
    ...
    public var defaultValue: AssetInternalValue {
        switch self {
    ...
    public func coerceToSyntax(_ value: AssetInternalValue) -> AssetInternalValue? {
    ...
    private func coerceToBoolean(_ value: AssetInternalValue) -> AssetInternalValue? {
        ...
        case .bytes(let bytes):
            let bytesAsBigUInt = BigUInt(bytes)
            return coerceToBoolean(.int(BigInt(bytesAsBigUInt)))
        case .bool:
            return value
        case .generalisedTime, .subscribable, .openSeaNonFungibleTraits:
            return nil
