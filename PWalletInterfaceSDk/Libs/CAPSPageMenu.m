...
NSString * const CAPSPageMenuOptionSelectionIndicatorLength             = @"selectionIndicatorLength";
...
NSString * const CAPSPageMenuOptionHideStartingMenuMargin               = @"startingMenuMargin";
NSString * const CAPSPageMenuOptionHideIndicator                        = @"hideIndicator";
...
- (instancetype)initWithViewControllers:(NSArray *)viewControllers frame:(CGRect)frame options:(NSDictionary *)options
{
    ...
    if (options) {
        for (NSString *key in options) {
            ...
            } else if ([key isEqualToString:CAPSPageMenuOptionSelectionIndicatorLength]){
                _selectionIndicatorLength = options[key];
            ...
            } else if ([key isEqualToString:CAPSPageMenuOptionHideStartingMenuMargin]){
                _startingMenuMargin = [options[key] floatValue];
            }else if ([key isEqualToString:CAPSPageMenuOptionHideIndicator]){
                _hideIndicator = [options[key] boolValue];
            }
        }
...
- (void)initValues
{
    ...
    _menuItemFont = [UIFont boldSystemFontOfSize:15.0];
    _menuSelectedItemFont = [UIFont boldSystemFontOfSize:15.0];
...
- (void)configureUserInterface
{
    ...
    for (UIViewController *controller in _controllerArray) {
        ...
        if (controller.navigationItem.title != nil) {
            [menuItemView setTitleText:controller.navigationItem.title];
        } else {
    ...
    if (_useMenuLikeSegmentedControl) {
        ...
        if (_centerMenuItems) {
            ...
            if ([_selectionIndicatorLength isEqualToString:@"half"]) {
                selectionIndicatorFrame = CGRectMake(_startingMenuMargin + _menuMargin, _menuHeight - _selectionIndicatorHeight, _menuItemWidth/2, _selectionIndicatorHeight);
            }
            if ([_selectionIndicatorLength isEqualToString:@"centerHalf"]) {
                selectionIndicatorFrame = CGRectMake(_startingMenuMargin + _menuMargin + _menuItemWidth / 4, _menuHeight - _selectionIndicatorHeight, _menuItemWidth / 2, _selectionIndicatorHeight);
            }
        } else {
    ...
    _selectionIndicatorView.layer.masksToBounds = YES;
    _selectionIndicatorView.layer.cornerRadius = 2;
    _selectionIndicatorView.backgroundColor = _selectionIndicatorColor;
    _selectionIndicatorView.hidden = _hideIndicator;
...
- (void)moveSelectionIndicator:(NSInteger)pageIndex
{
    if (pageIndex >= 0 && pageIndex < _controllerArray.count) {
        [UIView animateWithDuration:0.15 animations:^{
            ...
            if ([self.selectionIndicatorLength isEqual: @"centerHalf"]) {
                selectionIndicatorX = selectionIndicatorX + selectionIndicatorWidth / 2;
            }
    ...
    if (tappedPoint.y < self.menuScrollView.frame.size.height) {
        ...
        if (itemIndex >= 0 && itemIndex < _controllerArray.count) {
            ...
            [UIView animateWithDuration:duration animations:^{
                CGFloat xOffset = (CGFloat)itemIndex * self.controllerScrollView.frame.size.width;
                [self.controllerScrollView setContentOffset:CGPointMake(xOffset, self.controllerScrollView.contentOffset.y)];
            }];
...
- (void)addPageAtIndex:(NSInteger)index
{
    ...
    CGFloat y = _menuHeight - 20;
    if(@available(iOS 11.0, *))
    {
        y = _menuHeight;
    }
    newVC.view.frame = CGRectMake(self.view.frame.size.width * (CGFloat)index, y, self.view.frame.size.width, self.view.frame.size.height - _menuHeight);
 ...
 */
- (void)moveToPage:(NSInteger)index object:(NSDictionary *)param
{
    if (index >= 0 && index < _controllerArray.count) {
        // Update page if changed
        ...
        if (param && [param objectForKey:@"move"])
        {
