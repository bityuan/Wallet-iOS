...
- (void)createView {
    ...
    launchTitleLab.text = @"转账".localized;
    ...
    titleLab.text = [NSString stringWithFormat:@"  %@",@"收币地址".localized];
...
//    personBtn.hidden = YES;
    ...
    addressLab.placeholder = @"输入/扫一扫地址".localized;
...
- (void)setCoin:(LocalCoin *)coin {
    _coin = coin;
    NSString *title = @"转账".localized;
    self.launchTitleLab.text = [NSString stringWithFormat:@"%@%@",coin.optional_name,title];
    self.addressText.placeholder = [NSString stringWithFormat:@"%@%@%@",@"输入/扫一扫".localized,coin.optional_name,@"地址".localized];
...
- (void)setContact:(PWContacts *)contact
{
    _contact = contact;
    self.nameLab.text = contact.nickName;
    [self.nameLab setHidden:NO];
    NSArray *addressArray = contact.coinArray;
    
    if (_index == 10000)
    {
        for (PWContactsCoin *coin in addressArray) {
            if ([coin.coinName isEqualToString:_coin.coin_type]) {
                self.addressText.text = coin.coinAddress;
                NSMutableAttributedString *resultAttr = [[NSMutableAttributedString alloc] initWithString:self.addressText.text];
    ...
    {
        PWContactsCoin *coin = [addressArray objectAtIndex:_index];
        self.addressText.text = coin.coinAddress;
        NSMutableAttributedString *resultAttr = [[NSMutableAttributedString alloc] initWithString:self.addressText.text];
        if (self.addressText.text.length > 4) {
            [resultAttr addAttribute:NSForegroundColorAttributeName value:MainColor range:NSMakeRange(self.addressText.text.length - 4, 4)];
...
//
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (self.midViewBlock) {
            self.midViewBlock(self.addressText.text);
        }
    });
}
