...
- (void)initView
{
    ...
    [self.knowBtn setTitle:@"已知晓 (3s)".localized forState:UIControlStateNormal];
...
- (void)timeDown
{
    _secondsCountDown-- ;
    self.knowBtn.enabled = NO;
    [self.knowBtn setTitle:[NSString stringWithFormat:@"已知晓 (%lis)".localized,_secondsCountDown] forState:UIControlStateNormal];
    self.knowBtn.backgroundColor = CMColorFromRGB(0xc3c9d1);
    if (_secondsCountDown <= 0) {
        self.knowBtn.enabled = YES;
        [self.knowBtn setTitle:[NSString stringWithFormat:@"已知晓".localized] forState:UIControlStateNormal];
