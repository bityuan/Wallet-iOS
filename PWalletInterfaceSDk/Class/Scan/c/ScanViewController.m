...
- (void)viewDidLoad {
    ...
    [PWAuthorizationTool detectionCameraAuthorized:^{
        ...
        UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:@"摄像头".localized message:@"请前往设置打开摄像头权限".localized preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *action1 = [UIAlertAction actionWithTitle:@"确定".localized style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:UIApplicationOpenSettingsURLString] options:@{} completionHandler:nil];
           
            [self.navigationController popViewControllerAnimated:false];
        }];
        UIAlertAction *action2 = [UIAlertAction actionWithTitle:@"取消".localized style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
...
- (void)createBackBtn
{
    ...
    CGFloat y = kTopOffset - 64 + 20;
    
    UILabel *titleLab = [[UILabel alloc] init];
    titleLab.text = @"扫一扫".localized;
    ...
    [photosBtn setTitle:@"相册".localized forState:UIControlStateNormal];
...
- (void)rightBarButtonItenAction {
    ...
    [PWAuthorizationTool detectionPhotoAuthorized:^{
        ...
        UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:@"相册".localized message:@"请前往设置打开相册访问权限".localized preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *action1 = [UIAlertAction actionWithTitle:@"确定".localized style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:UIApplicationOpenSettingsURLString] options:@{} completionHandler:nil];
        }];
        UIAlertAction *action2 = [UIAlertAction actionWithTitle:@"取消".localized style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
...
- (void)openPhoto {
    ...
    [obtain setBlockWithQRCodeObtainAlbumResult:^(SGQRCodeObtain *obtain, NSString *result) {
        if (result == nil) {
            [weakSelf showCustomMessage:@"未发现二维码/条形码".localized hideAfterDelay:1];
        } else {
            if (weakSelf.scanResult) {
                [obtain stopRunning];
                [weakSelf.navigationController popViewControllerAnimated:true];
...
- (UILabel *)promptLabel {
    if (!_promptLabel) {
        ...
        _promptLabel.text = @"请将镜头对准二维码进行扫描".localized;
    }
    return _promptLabel;
}
