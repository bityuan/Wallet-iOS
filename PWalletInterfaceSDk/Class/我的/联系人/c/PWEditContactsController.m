...
@interface PWEditContactsController ()<UITableViewDelegate,UITableViewDataSource,PWAddContactsAddressCellDelegate>
//tableView
...
@implementation PWEditContactsController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"编辑联系人".localized;
    
    if (self.contacts) {
        self.nickName = self.contacts.nickName;
        self.phoneNumber = self.contacts.phoneNumber;
        self.coinArray = [NSMutableArray arrayWithArray:self.contacts.coinArray];
    }
...
- (void)saveButtonPress {
    ...
    contacts.contcatId = self.contacts.contcatId;
    contacts.nickName = self.nickName;
    contacts.phoneNumber = self.phoneNumber;
    contacts.coinArray = self.coinArray;
    //更新本地存储数据
    [[PWContactsTool shared]updateOldContacts:self.contacts newContacts:contacts];
    for (UIViewController *vc in self.navigationController.viewControllers) {
        if ([vc isKindOfClass:[PWContactsDetailController class]]) {
            PWContactsDetailController *contactsDetailVC = (PWContactsDetailController*)vc;
            contactsDetailVC.contacts = contacts;
            [self.navigationController popToViewController:contactsDetailVC animated:true];
            return;
        }
    }
...
- (void)setSaveButtonState {
    ...
    for (PWContactsCoin *coin in self.coinArray) {
        if ((coin.coinName == nil) || (coin.coinAddress == nil) || ([coin.coinName  isEqual: @""]) || ([coin.coinAddress  isEqual: @""])) {
            self.saveBtn.userInteractionEnabled = false;
            self.saveBtn.alpha = 0.4;
            return;
        }
...
- (void)addContactsAddressCell:(PWAddContactsAddressCell *)cell addressTextViewDidEndEditing:(UITextView *)textView {
    ...
    self.coinArray[row].coinAddress = textView.text;
    [self setSaveButtonState];
}
//主链(虚拟币名字)点击
...
- (void)addContactsAddressCell:(PWAddContactsAddressCell *)cell scanBtnPress:(UIButton *)button {
    ...
    scanVC.scanResult = ^(NSString *address) {
        if ([address containsString:@","]) {
            ...
            if (array.count == 3) {
                cell.addressText = [NSString stringWithFormat:@"%@",array[2]];
                NSInteger row = [weakSelf.tableView indexPathForCell:cell].row;
                weakSelf.coinArray[row].coinAddress = [NSString stringWithFormat:@"%@",array[2]];
                [weakSelf setSaveButtonState];
            }
            else if (array.count == 2)
            {
                NSString *add = [NSString stringWithFormat:@"%@",array[1]];
                cell.addressText = add;
            }
        }
        else
        {
