...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (![self.topTitle isEqualToString:@"手续费".localized]) {
    ...
    if (self.exchangeHandler) {
        if(self.showSlider)
        {
            ...
            if (self.balance < feeAndAm) {
                [self showCustomMessage:@"矿工费不足".localized hideAfterDelay:2.f];
                return;
            }
        }
        [self dismissViewControllerAnimated:false completion:^{
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if([self.topTitle isEqualToString:@"修改钱包名称".localized  ])
    {
    ...
    if([self.topTitle isEqualToString:@"修改用户名".localized  ])
       {
           NSString *finalStr = [textField.text stringByAppendingString:string];
...
- (void)initSubViews {
    ...
    ftitleLab.text = [NSString stringWithFormat:@"  %@",@"矿工费".localized  ];
    ...
    if ([_topTitle isEqualToString:@"请输入钱包密码".localized  ]) {
    ...
    {
        if ([self.topTitle isEqualToString:@"请输入转币数量".localized  ])
        {
            textField.keyboardType = UIKeyboardTypeDecimalPad;
            textField.placeholder = @"请输入转币数量".localized  ;
        }
        else
        {
            textField.keyboardType = UIKeyboardTypeDefault;
