...
- (void)loadData {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSDictionary *dic = [GoFunction queryTransactionByTxid:self.coinType platform:self.platStr Txid:self.tradeHash andTreaty:weakSelf.coin.treaty];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (IS_BLANK(dic)) {
                [self showCustomMessage:@"网络请求失败".localized hideAfterDelay:1];
            } else {
                [self createView];
                [self setViewValue];
                [self hideProgress];
...
- (void)createView
{
    ...
    blockInfoLab.text = @"区块信息".localized;
    ...
    remarksLab.text = @"上链备注".localized;
    ...
    remarksValueLab.text = IS_BLANK(self.orderModel.note) ? @"无".localized : self.orderModel.note;
    ...
    {
        self.remarksValueLab.text = @"无".localized;
    }
    
    //添加联系人
    UIButton *addContactBtn = [[UIButton alloc] init];
    [addContactBtn setTitle:@"添加联系人".localized forState:UIControlStateNormal];
...
- (void)setViewValue
{
    ...
    self.title = [NSString stringWithFormat:@"%@%@",self.coin.optional_name,@"交易详情".localized];
...
- (void)addFriendAction
{
    if ([_orderModel.type isEqualToString:@"send"]) {
        ...
        if([self.stateLab.text containsString:@":"])
        {
            [self showCustomMessage:@"已经是好友".localized hideAfterDelay:2.0];

        }else{
            PWAddContactsController *vc = [[PWAddContactsController alloc] init];
            vc.coinType = _coin.coin_type;
            vc.address = _orderModel.toAddress;
            vc.optional_name = _coin.optional_name;
    ...
    }else
    {
        if([self.stateLab.text containsString:@":"])
        {
            ...
            PWAddContactsController *vc = [[PWAddContactsController alloc] init];
            vc.coinType = _coin.coin_type;
            vc.address = _orderModel.fromAddress;
            vc.optional_name = _coin.optional_name;
            [self.navigationController pushViewController:vc
                                                 animated:YES];
        }
...
- (void)getContactsWithAddress:(OrderModel *)orderModel
{
    if (orderModel.state == TransferFromStatusFailure) {
        ...
        self.stateLab.text = [@"交易失败".localized stringByAppendingString:@""];
    ...
    }else if(orderModel.state == TransferFromStatusEnsureing)
    {
        ...
        self.stateLab.text = [@"确认中".localized stringByAppendingString:@""];
    ...
    }else if (orderModel.state == TransferFromStatusSuccess)
    {
        ...
        self.stateLab.text = [@"交易成功".localized stringByAppendingString:@""];
