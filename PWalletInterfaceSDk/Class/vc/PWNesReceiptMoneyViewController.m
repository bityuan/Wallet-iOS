...
- (void)createView{
    ...
    //背景
    UIImageView *bgView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"收款背景"]];
    ...
    [backBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.view).offset(kTopOffset - 32);
    ...
    titleLabel.text = [NSString stringWithFormat:@"%@%@",_coin.optional_name,@"收款".localized];
    ...
    UILabel *moneyLab = [UILabel getLabWithFont:[UIFont systemFontOfSize:22]
                                  ...
                                  textAlignment:NSTextAlignmentCenter
                                           text:[NSString stringWithFormat:@"%@ %@",@"请转入".localized,_coin.optional_name]];
    [self.view addSubview:moneyLab];
    self.moneyLab = moneyLab;
    
    UIButton *moneyBtn = [[UIButton alloc] init];
    [moneyBtn setTitle:@"指定金额".localized forState:UIControlStateNormal];
    ...
    balanceLab.text = [NSString stringWithFormat:@"%@：%@%@",@"余额".localized,balanceStr,_coin.optional_name];
    ...
    [qrcodeImg setImage:[CommonFunction createImgQRCodeWithString:_coin.coin_address centerImage:tempImageView.image]];
    ...
    CGFloat topSpace = kTopOffset + 40;
    if (isIPhone5) {
        topSpace = kTopOffset + 10;
    }
...
- (void)changeType:(UIButton *)sender
{
    switch (sender.tag) {
        ...
        {
            _receiptQrcodeType = ReceiptQrcodeTypeUniseral;
            self.tipLab.text = @"适用于所有钱包收款".localized;
        ...
        case 200:
        {
            ...
            CGSize size = [[NSString stringWithFormat:@"仅适用于%@收款".localized,@""] sizeWithAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:12.f],NSFontAttributeName,nil]];
            self.tipLab.text = [NSString stringWithFormat:@"仅适用于%@收款".localized,@""];
            [self.tipLab mas_remakeConstraints:^(MASConstraintMaker *make) {
...
{
    WEAKSELF
    PWAlertController *alertVC = [[PWAlertController alloc] initWithTitle:@"请输入转币数量".localized withTextValue:@""  leftButtonName:nil rightButtonName:@"确定".localized handler:^(ButtonType type, NSString *text) {
        if (type == ButtonTypeRight) {
            ...
            weakSelf.moneyLab.text = [NSString stringWithFormat:@"%@ %@ %@",@"指定收款".localized,weakSelf.moneyStr,weakSelf.coin.optional_name];
            ...
            [weakSelf.moneyBtn setTitle:@"修改金额".localized forState:UIControlStateNormal];
...
- (void)eyeBtnClickAction:(UIButton *)sender
{
    if([sender isSelected])
    {
        ...
        self.balanceLab.text = [NSString stringWithFormat:@"%@：%@%@",@"余额".localized,balanceStr,_coin.optional_name];
    }else{
        [sender setSelected:YES];
        self.balanceLab.text = [NSString stringWithFormat:@"%@：****%@",@"余额".localized,_coin.optional_name];
        self.balanceLab.text = [NSString stringWithFormat:@"%@：****%@",@"余额".localized,_coin.optional_name];
        ...
        dispatch_sync(dispatch_get_main_queue(), ^{
            if (![weakSelf.eyeBtn isSelected]) {
                ...
                if (balance != -1)
                {
                    ...
                    weakSelf.balanceLab.text = [NSString stringWithFormat:@"%@：%@%@",@"余额".localized,balanceStr,weakSelf.coin.optional_name];
                }
            }
