...
- (void)viewDidLoad {
    ...
    _coinTypeArray = @[@"全部".localized,@"转账".localized,@"收款".localized];
    self.contactInfoArray = [[PWContactsTool shared] getAllContacts];
...
- (void)createView{
    ...
    blackView.backgroundColor = SGColor(51, 54, 73);
    ...
    self.titleView.nfturlBlock = ^{
        if (weakSelf.nftUrl != nil) {
            ...
            [weakSelf showCustomMessage:@"未获取到对应的NFT地址 请稍后再试".localized hideAfterDelay:2.f];
        }
    ...
    [outBtn setTitle:@"转账".localized forState:UIControlStateNormal];
    ...
    [inBtn setTitle:@"收款".localized forState:UIControlStateNormal];
    ...
    [tradeTitle setTitle:[NSString stringWithFormat:@"  %@",@"交易记录".localized] forState:UIControlStateNormal];
    ...
    [toBroswerBtn setTitle:@"前往区块链浏览器".localized forState:UIControlStateNormal];
    ...
    //没有交易记录
    NoRecordView *noRecordView = [[NoRecordView alloc] initWithImage:[UIImage imageNamed:@"notraderecord"] title:@"暂无交易记录".localized];
...
-(void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(_transactionArray.count > 0)
    {
        ...
        tradeCell.contactInfoArray = self.contactInfoArray;
...
- (void)moneyOutAction{
    ...
    if(self.localWallet.wallet_issmall == 3){
        [self showCustomMessage:@"暂不支持".localized hideAfterDelay:2.f];
        return;
    }
...
- (void)imageZoomAction:(UIImageView *)imageView
{
    ...
    PWImageAlertView *imageAlerView = [[PWImageAlertView alloc]initWithTitle:@"收款地址".localized image:imageView.image address:self.selectedCoin.coin_address];
    WEAKSELF
